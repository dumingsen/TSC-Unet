...
def create_directory(directory_path):#创建路径
...
def read_dataset(root_dir, archive_name, dataset_name):
    datasets_dict = {}#字典
    cur_root_dir = root_dir.replace('-temp', '')#空
    ...
    if archive_name == 'mts_archive':
        ...
        x_train, y_train = readucr(file_name + '_TRAIN.txt') ##33
...
def read_all_datasets(root_dir, archive_name, split_val=False):
    ...
    if archive_name == 'mts_archive':
        ...
        for dataset_name in DATASET_NAMES:
            ...
            x_train, y_train = readucr(file_name + '_TRAIN')#读取数据
