...
class Classifier_UNET:
    ...
    def __init__(self, output_directory, input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes,
        ...
        if build == True:
            ...
            #self.model = self.build_model1(input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes)
    ...
    def build_model2(self, input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes):
        ...
        con1 = Conv1D(128, 8, padding='same')(input_pre1)  # 2d
        con1 = keras.layers.BatchNormalization()(con1)
        con1 = keras.layers.Activation('relu')(con1)
        # con1 = Conv1D(256, 5, padding='same')(con1)  # 2d
        # con1 = keras.layers.BatchNormalization()(con1)
        # con1 = keras.layers.Activation('relu')(con1)
        # con1 = Conv1D(128, 3, padding='same')(con1)  # 2d
        # con1 = keras.layers.BatchNormalization()(con1)
        # con1 = keras.layers.Activation('relu')(con1)
        ...
        # 每一次下采样后我们都把特征通道的数量加倍
        # 每次重复都有两个输出：一个用于编码部分进行特征提取，一个用于解码部分的特征融合
        conv1 = Conv1D(a, 2*aa, padding='same')(output_combine)  # 需要的是拼接后的特征,, kernel_regularizer=regularizers.l2(e)
        conv1 = keras.layers.BatchNormalization()(conv1)
        conv1 = keras.layers.Activation('relu')(conv1)
        ...
        print('pool1', pool1.shape)
        # 2
        conv2 = Conv1D(2 * a, 2*aa, padding='same')(pool1)
        conv2 = keras.layers.BatchNormalization()(conv2)
        conv2 = keras.layers.Activation('relu')(conv2)
        ...
        print('2', pool2.shape)
        # 3
        conv3 = Conv1D(4 * a, 2*aa, padding='same')(pool2)
        conv3 = keras.layers.BatchNormalization()(conv3)
        conv3 = keras.layers.Activation('relu')(conv3)
        ...
        # 4，两个3x3卷积，2x2最大池化
        conv4 = Conv1D(8 * a, 2*aa, activation='relu', padding='same')(pool3)  # ,kernel_regularizer=regularizers.l2(e)
        conv4 = keras.layers.BatchNormalization()(conv4)
        conv4 = keras.layers.Activation('relu')(conv4)
    ...
    #桥变残差
    def build_modelbridge(self, input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes):
        ##使用二维卷积
        c=1
        #c = 3
        # # 1 原始序列提取特征
        ...
        # 右侧解码部分
        # 每个重复结构前先使用反卷积，每次反卷积后特征通道数量减半，特征图大小加倍（绿箭头）
        conv6 = Conv2D(8 * a, 2, padding='same')(output_block_1)  # ,kernel_regularizer=regularizers.l2(e))(drop5
        print('up6', conv6.shape)
        conv6 = keras.layers.BatchNormalization()(conv6)
        conv6 = keras.layers.Activation('relu')(conv6)
        up6 = UpSampling2D(size=(2, 2))(conv6)
    ...
    def build_modelno(self, input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes):
        ##使用二维卷积
        c=1
        #c = 3
        # # 1 原始序列提取特征
    ...
    def fit(self, x_train, x_trainn1, x_trainn2, x_trainn3, x_trainn4, y_train, x_test,x_testn1, x_testn2, x_testn3, x_testn4,
            y_val,
