...
class Classifier_UNET:
    ...
    ####老的
    def build_modelo(self, input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes):
        ##使用二维卷积
        c=2
        a = 16
        #c = 3
        ...
        n_feature_maps=a
        #1
        conv_x = keras.layers.Conv2D(filters=n_feature_maps, kernel_size=2, padding='same')(output_combine)#输入
        conv_x = keras.layers.BatchNormalization()(conv_x)
        conv_x = keras.layers.Activation('relu')(conv_x)
        #2
        conv_y = keras.layers.Conv2D(filters=n_feature_maps, kernel_size=2, padding='same')(conv_x)
        ...
        # expand channels for the sum
        shortcut_y = keras.layers.Conv2D(filters=n_feature_maps, kernel_size=1, padding='same')(output_combine)#输入
        shortcut_y = keras.layers.BatchNormalization()(shortcut_y)
        #作和
        output_block_1 = keras.layers.add([shortcut_y, conv_z])
        output_combine = keras.layers.Activation('relu')(output_block_1)
    ...
    def build_model2(self, input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes):
        ...
        con1 = Conv1D(d, 10, padding='valid')(input_pre1)  # 2d
        con1 = keras.layers.BatchNormalization()(con1)
        con1 = keras.layers.Activation('relu')(con1)
        con1 = Conv1D(d, 8, padding='valid')(con1)  # 2d
        con1 = keras.layers.BatchNormalization()(con1)
        con1 = keras.layers.Activation('relu')(con1)
    ...
    def fitt(self, x_train, x_trainn1, x_trainn2, x_trainn3, x_trainn4, y_train,x_test, x_testn1, x_testn2, x_testn3, x_testn4,
            y_val,
       ...
       # c = 3
        ...
        if c == 3:
            ...
            hist = self.model.fit({'input1': x_trainn1},
                                  {'output1': y_train}, batch_size=mini_batch_size,
                                  validation_data=([x_testn1], y_val),
                                  epochs=nb_epochs,
                                  verbose=self.verbose,
                                  callbacks=self.callbacks)  # model.fit()方法用于执行训练过程
        elif c == 2:
