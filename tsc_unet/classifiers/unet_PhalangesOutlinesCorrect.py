...
class Classifier_UNET:
    ...
    def __init__(self, output_directory, input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes,
        ...
        if build == True:
            # c = 3  # 标志
            #参数别改
            self.model = self.build_model2a(input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes)
    ...
    def build_model2(self, input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes):
        ...
        con1 = Conv1D(128, 4, padding='same')(input_pre1)  # 2d
        con1 = keras.layers.BatchNormalization()(con1)
        con1 = keras.layers.Activation('relu')(con1)
        con1 = Conv1D(256, 4, padding='same')(con1)  # 2d
        con1 = keras.layers.BatchNormalization()(con1)
        con1 = keras.layers.Activation('relu')(con1)
        ...
        return model
    #
    def build_model2a(self, input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes):
        ##使用二维卷积
        c = 1
        b = 2
        d =32
        ...
        # 每一次下采样后我们都把特征通道的数量加倍
        # 每次重复都有两个输出：一个用于编码部分进行特征提取，一个用于解码部分的特征融合
        conv1 = Conv1D(a, 4*aa, padding='same')(output_combine)  # 需要的是拼接后的特征,, kernel_regularizer=regularizers.l2(e)
        print('conv1', conv1.shape)
        conv1 = keras.layers.BatchNormalization()(conv1)
        conv1 = keras.layers.Activation('relu')(conv1)
        conv1 = Conv1D(a,2* aa, padding='same')(conv1)  # , kernel_regularizer=regularizers.l2(e)
        print('conv1', conv1.shape)
        conv1 = keras.layers.BatchNormalization()(conv1)
        conv1 = keras.layers.Activation('relu')(conv1)
        # conv1=Dropout(0.5)(conv1)
        ...
        up9 = UpSampling1D(size=2)(conv9)
        merge9 = concatenate([conv1, up9], axis=2)
        conv9 = Conv1D(a, 4*aa, padding='same')(merge9)
        conv9 = keras.layers.BatchNormalization()(conv9)
        conv9 = keras.layers.Activation('relu')(conv9)
        conv9 = Conv1D(a, 2*aa, padding='same')(conv9)
        conv9 = keras.layers.BatchNormalization()(conv9)
        conv9 = keras.layers.Activation('relu')(conv9)
        # conv9 = Conv1D(a, 2,padding='same')(conv9)
        # conv9 = keras.layers.BatchNormalization()(conv9)
