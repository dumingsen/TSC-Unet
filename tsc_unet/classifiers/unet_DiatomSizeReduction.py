...
class Classifier_UNET:
    ...
    def __init__(self, output_directory, input_pre1, input_pre2, input_pre3, input_pre4,input_pre5, nb_classes, verbose=True,
                 build=True):  # verbose false
        self.output_directory = output_directory
        if build == True:
            self.model = self.build_model2(input_pre1, input_pre2, input_pre3, input_pre4,input_pre5, nb_classes)
            if (verbose == True):
                self.model.summary()
    ...
    def build_model2(self, input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes):
        ...
        con1 = Conv1D(d, 12, padding='valid')(input_pre1)  # 2d
        con1 = keras.layers.BatchNormalization()(con1)
        con1 = keras.layers.Activation('relu')(con1)
        con1 = Conv1D(d, 9, padding='valid')(con1)  # 2d
        con1 = keras.layers.BatchNormalization()(con1)
        con1 = keras.layers.Activation('relu')(con1)
        ...
        return model
    def build_model(self, input_pre1, input_pre2, input_pre3, input_pre4, nb_classes):
        ##使用二维卷积
        ...
        # 二维序列特征提取1
        input_pre2 = Input(input_pre2[1:], name='input1')
        con2 = Conv2D(32, 2, padding='same')(input_pre2)  # 2d
        con2 = keras.layers.BatchNormalization()(con2)
        con2 = keras.layers.Activation('relu')(con2)
        output2 = Model(inputs=input_pre2, outputs=con2)
        print('二维序列1', con2.shape)
        ...
        # 二位特征提取2
        input_pre3 = Input(input_pre3[1:], name='input2')
        con3 = Conv2D(32, 2, padding='same')(input_pre3)
        con3 = keras.layers.BatchNormalization()(con3)
        con3 = keras.layers.Activation('relu')(con3)
        con3 = MaxPooling2D(pool_size=(2, 2))(con3)
        output3 = Model(inputs=input_pre3, outputs=con3)
        print('二维序列2', con3.shape)

        # 二维特征提取3
        input_pre4 = Input(input_pre4[1:], name='input3')
        con4 = Conv2D(32, 2, padding='same')(input_pre4)
        con4 = keras.layers.BatchNormalization()(con4)
        con4 = keras.layers.Activation('relu')(con4)
        output4 = Model(inputs=input_pre4, outputs=con4)
        print('二维序列3', con4.shape)
        ...
        # 每一次下采样后我们都把特征通道的数量加倍
        # 每次重复都有两个输出：一个用于编码部分进行特征提取，一个用于解码部分的特征融合
        conv1 = Conv2D(a, 2, padding='same')(output_combine)  # 需要的是拼接后的特征
        print('conv1', conv1.shape)
        conv1 = keras.layers.BatchNormalization()(conv1)
        conv1 = keras.layers.Activation('relu')(conv1)
        conv1 = Conv2D(a, 2, padding='same')(conv1)
        print('conv1', conv1.shape)
        conv1 = keras.layers.BatchNormalization()(conv1)
        conv1 = keras.layers.Activation('relu')(conv1)
        ...
        conv2 = keras.layers.BatchNormalization()(conv2)
        conv2 = keras.layers.Activation('relu')(conv2)
        conv2 = Conv2D(2 * a, 2, padding='same')(conv2)
        conv2 = keras.layers.BatchNormalization()(conv2)
        conv2 = keras.layers.Activation('relu')(conv2)
        ...
        conv3 = keras.layers.BatchNormalization()(conv3)
        conv3 = keras.layers.Activation('relu')(conv3)
        conv3 = Conv2D(4 * a, 2, padding='same')(conv3)
        conv3 = keras.layers.BatchNormalization()(conv3)
        conv3 = keras.layers.Activation('relu')(conv3)
        pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)
        print('3', pool3.shape)
        # 4，两个3x3卷积，2x2最大池化
        conv4 = Conv2D(8 * a, 2, activation='relu', padding='same')(pool3)
        conv4 = keras.layers.BatchNormalization()(conv4)
        conv4 = keras.layers.Activation('relu')(conv4)
        conv4 = Conv2D(8 * a, 2, activation='relu', padding='same')(conv4)
        conv4 = keras.layers.BatchNormalization()(conv4)
        conv4 = keras.layers.Activation('relu')(conv4)
        drop4 = Dropout(0.5)(conv4)  # 3, 2, 512
        print('drop4', drop4.shape)
        ...
        # 5.将编码部分和解码部分组合一起，就可构建UNet网络，
        # 在这里UNet网络的深度通过depth进行设置，并设置第一个编码模块的卷积核个数通过filter进行设置
        conv5 = Conv2D(16 * a, 2, padding='same')(pool4)
        conv5 = keras.layers.BatchNormalization()(conv5)
        conv5 = keras.layers.Activation('relu')(conv5)
        ...
        # 右侧解码部分
        # 每个重复结构前先使用反卷积，每次反卷积后特征通道数量减半，特征图大小加倍（绿箭头）
        conv6 = Conv2D(8 * a, 2, padding='same')(drop5)  # (            )#2, 2, 512
        print('up6', conv6.shape)
        conv6 = keras.layers.BatchNormalization()(conv6)
        conv6 = keras.layers.Activation('relu')(conv6)
        ...
        conv6 = keras.layers.BatchNormalization()(conv6)
        conv6 = keras.layers.Activation('relu')(conv6)
        conv6 = Conv2D(8 * a, 2, padding='same')(conv6)
        conv6 = keras.layers.BatchNormalization()(conv6)
        conv6 = keras.layers.Activation('relu')(conv6)
        print('conv6', conv6.shape)

        conv7 = Conv2D(4 * a, 2, padding='same')(conv6)  # ()
        conv7 = keras.layers.BatchNormalization()(conv7)
        conv7 = keras.layers.Activation('relu')(conv7)
        up7 = UpSampling2D(size=(2, 2))(conv7)
        merge7 = concatenate([conv3, up7], axis=3)
        conv7 = Conv2D(4 * a, 2, padding='same')(merge7)
        conv7 = keras.layers.BatchNormalization()(conv7)
        conv7 = keras.layers.Activation('relu')(conv7)
        ...
        conv8 = Conv2D(2 * a, 2, padding='same')(conv7)
        conv8 = keras.layers.BatchNormalization()(conv8)
        conv8 = keras.layers.Activation('relu')(conv8)
        ...
        conv9 = Conv2D(a, 2, padding='same')(conv8)
        ...
        conv9 = keras.layers.BatchNormalization()(conv9)
        conv9 = keras.layers.Activation('relu')(conv9)
        # conv9 = Conv2D(2, 2, activation='relu', padding='same'  )(conv9)
        # conv9 = keras.layers.BatchNormalization()(conv9)
        # conv9 = keras.layers.Activation('relu')(conv9)
        ...
        gap_layer = keras.layers.GlobalAveragePooling2D()(conv9)  # 出错
        print('gap', gap_layer.shape)
        output_layer = keras.layers.Dense(nb_classes, activation='softmax', name='output1')(gap_layer)
        print('out', output_layer.shape)
        ...
        # optimizer = 优化器，loss = 损失函数， metrics = ["准确率”])
        # 多分裂交叉熵，，
        reduce_lr = keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=20, mode='auto',
                                                      min_lr=0.0001)
        # monitor：要监测的数量。factor：学习速率降低的因素。new_lr = lr * factor。patience：没有提升的epoch数，之后学习率将降低。min_lr：学习率的下限。
    ...
    def drow(self, history, output_directory):
        ...
        plt.show()
    def fitt(self, x_train, x_trainn1, x_trainn2, x_trainn3, y_train,x_test, x_testn1, x_testn2, x_testn3, y_val,
        ...
        mini_batch_size = int(min(math.ceil(x_train.shape[0] / 10), batch_size))
        ...
        y_pred = model.predict([x_testn1, x_testn2, x_testn3])  # 预测
    ...
    def fit(self, x_train, x_trainn1, x_trainn2, x_trainn3, x_trainn4, y_train, x_test, x_testn1, x_testn2, x_testn3,
        ...
        print('123', x_trainn1.shape)  # [x_trainn1, x_trainn2, x_trainn3], y_train
        if c == 3:
            hist = self.model.fit({'input0': x_train, 'input1': x_trainn1, 'input2': x_trainn2},
                                  {'output1': y_train}, batch_size=mini_batch_size,
                                  validation_data=([x_test, x_test, x_test], y_val),
                                  epochs=nb_epochs,
                                  verbose=self.verbose,
                                  callbacks=self.callbacks)  # model.fit()方法用于执行训练过程
        elif c == 1:
            ...
            hist = self.model.fit({'input0': x_train, 'input1': x_trainn1},
                                  {'output1': y_train}, batch_size=mini_batch_size,
                                  validation_data=([x_test, x_test], y_val),
                                  epochs=nb_epochs,
                                  verbose=self.verbose,
                                  callbacks=self.callbacks)  # model.fit()方法用于执行训练过程
        elif c == 22:
            hist = self.model.fit({'input0': x_train, 'input1': x_trainn2},
                                  {'output1': y_train}, batch_size=mini_batch_size,
                                  validation_data=([x_test, x_test], y_val),
                                  epochs=nb_epochs,
                                  verbose=self.verbose,
