...
class Classifier_UNET:
    ...
    def build_model(self, input_pre1, input_pre2, input_pre3, input_pre4, input_pre5, nb_classes):
        ...
        n_feature_maps=64
        conv_x = keras.layers.Conv2D(filters=n_feature_maps, kernel_size=2, padding='same')(output_combine)#卷积核太大
        ...
        shortcut_y = keras.layers.Conv2D(filters=n_feature_maps, kernel_size=1, padding='same')(output_combine)
        ...
        conv_x = keras.layers.Conv2D(filters=n_feature_maps * 2, kernel_size=2, padding='same')(output_block_1)
        conv_x = keras.layers.BatchNormalization()(conv_x)
        conv_x = keras.layers.Activation('relu')(conv_x)

        conv_y = keras.layers.Conv2D(filters=n_feature_maps * 2, kernel_size=2, padding='same')(conv_x)
        conv_y = keras.layers.BatchNormalization()(conv_y)
        conv_y = keras.layers.Activation('relu')(conv_y)

        conv_z = keras.layers.Conv2D(filters=n_feature_maps * 2, kernel_size=2, padding='same')(conv_y)
        conv_z = keras.layers.BatchNormalization()(conv_z)
        ...
        conv_x = keras.layers.Conv2D(filters=n_feature_maps * 2, kernel_size=2, padding='same')(output_block_2)
        conv_x = keras.layers.BatchNormalization()(conv_x)
        conv_x = keras.layers.Activation('relu')(conv_x)
